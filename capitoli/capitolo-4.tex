...
\end{minted}
Dove in entrata avrà due tipi di flussi, uno relativo agli eventi da raggruppare di tipo \textit{BumblebeeOutput} (\S\ref{sec:pr1}) e uno relativo alla \textit{configurazione} della finestra temporale (\S\ref{sec:pr2}). Tale operatore operato su flussi suddivisi per chiave definita come \textbf{pipeID}. Infine il metodo che verrà richiamato alla fine di una specifica finestra temporale è il metodo \textit{onTimer} (\S\ref{sec:on-timer}), il quale collezionerà gli eventi all'interno della window appena conclusa e programmerà l'inizio della successiva finestra temporale.

\subsection{Flusso degli eventi da raggruppare}\label{sec:pr1}
...
\subsection{Flusso di configurazione della window}\label{sec:pr2}

\subsection{OnTimer}\label{sec:on-timer}
