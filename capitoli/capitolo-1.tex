...
Lo scopo dello stage e del relativo progetto è stato quello di creare una componente aggiuntiva per la piattaforma proprietaria \textbf{\textit{SYN}}. Tale componente si occupa dell'analisi ed elaborazione di dati in tempo reale provenienti da \textit{asset} differenti, i quali vengono raggruppati per perimetro geografico, categoria o tipologia. Tale componente, inoltre,
...
Le modifiche apportate rappresentano l'adattamento di tali operatori per rappresentare i vari \textit{asset} come un'entità raggruppata su cui poi andrà fatto un controllo mirato per definire se tale entità aggregata ha prodotto anomalie, rilevate attraverso \textit{detector} che sfruttano configurazioni definite dall'utente e/o algoritmi di \textit{\gls{Apprendimento automatico}}, come per esempio l'algoritmo \textit{\gls{xgboost}}.
...
\begin{risk}{Comprensione e utilizzo del framework \textit{Flink}}
    \riskdescription{durante lo studio e la creazione del \textit{\gls{poc}} per testare le principali funzionalità di \textit{Flink} sono stati riscontrati problemi relativi alle dipendenze utilizzate all'interno del codice}
    \risksolution{per risolvere tale problema è stato richiesta una supervisione da parte del tutor aziendale mirata a controllare il file che si occupa di gestire le dipendenze utilizzate all'interno del codice. Tramite l'aiuto del tutor aziendale è stato scoperto che venivano utilizzate, erroneamente, versioni molto datate dei relativi pacchetti e dipendenze relative a \textit{Java} e non a \textit{Scala}}
    \label{risk:flinkImport}
\end{risk}

\begin{risk}{Adattabilità operatore \textit{Bumblebee}}
    \riskdescription{l'operatore preesistente non riusciva nativamente a gestire un \textit{output} informativo che potesse trattare gruppi di risorse (eguali o diverse fra di loro)}
...
\begin{risk}{Adattabilità operatore \textit{Windowing}}
    \riskdescription{l'operatore temporale non riusciva a gestire contemporaneamente l'aggregazione e l'allineamento di gruppi di risorse (eguali o diverse fra di loro)}
...
\begin{risk}{Adattabilità operatore \textit{AlertCoProcess}}
    \riskdescription{l'operatore preesistente non riusciva a rilevare anomalie su gruppi di risorse differenti}
    \risksolution{si è deciso di modificare la logica di tale operatore adattandolo alle nuove esigenze, garantendo comunque la retrocompatibilità con il funzionamento precedente}
