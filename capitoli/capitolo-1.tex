...
\intro{Il documento seguente rappresenta il lavoro di tesi svolto come stage presso l'azienda Datasoil s.r.l. Lo stage è stato svolto al termine del percorso di studi della Laurea Triennale in Informatica presso l'Università degli Studi di Padova. Il progetto consiste nella realizzazione di una componente aggiuntiva al prodotto principale aziendale denominato SYN, dove tale componente si occupa di aggregazione e analisi in tempo reale di dati provenienti da asset differenti.}
...
Datasoil s.r.l. è una Startup Innovativa fondata da Pietro De Caro...\\
L'azienda si occupa di sviluppare prodotti \textit{Software as a Service} in cloud \gls{b2b} e \gls{b2c} e opera principalmente nell'ambito dell'analisi, processamento e fruizione di dati per supportare processi decisionali mirati da parte delle aziende e degli utenti. Fulcro di tale servizio è la piattaforma proprietaria \textbf{SYN}, la quale fornisce un visione coesa su processi ed infrastruttura derivata dall'analisi in tempo reale di dati provenienti da asset e dati esterni tramite algoritmi di \gls{Apprendimento automatico}, producendo alert in grado di evidenziarne eventi anomali e rilevanti.\\
L'azienda, inoltre, propone altri prodotti come l'applicativo LifeStyleSinc ... e l'utilizzo di realtà virtuale per...
...
si occupa di notificare l'utente finale tramite alert che vengono innescati dopo il superamento di specifiche soglie definite da regole decise dall'utente utilizzatore.
La realizzazione di tale componente ha previsto la modifica degli operatori preesistenti che si occupavano del raggruppamento temporale (\textit{windowing}) di asset eguali fra di loro e della componente di \textit{anomaly detection}, cioè quella che si occupa del rillevamento di anomalie.\\
Le modifiche apportate rappresentano l'addattamento di tale operatori per rappresentare i vari asset come un entità raggruppata su cui poi andrà fatto un controllo mirato per definire se tale entità (a livello globale) ha prodotto un'anomalia, dove il rillevamento di essa si basa su detector che sfruttano configurazioni definite dall'utente e/o algoritmi di \gls{Apprendimento automatico}, come per esempio l'algoritmo \gls{xgboost} di tipo \gls{gradient boosting}. La componente, oltre a sfruttare i detector preesistenti, utilizza anche detector nuovi definiti durante la realizzazione del progetto.
...
\begin{risk}{Comprensione e utilizzo del framework Flink}
    \riskdescription{Durante lo studio e la creazione del \gls{poc} per testare le principali funzionalità di Flink sono stati riscontrati problemi relativi alle dipendenze utilizzate all'interno del codice}
    \risksolution{Per risolvere tale problema è stato richiesta una supervisione da parte del tutor aziendale mirata a controllare il file che si occupa di gestire le dipendenze utilizzate all'interno del codice. Tramite l'aiuto del tutor aziendale è stato scoperto che erano state utilizzate versioni molto datate dei relativi pacchetti e inoltre venivano utilizzati dei pacchetti relativi a Java e non a Scala}
    \label{risk:flinkImport}
\end{risk}

\begin{risk}{Adattabilità operatore Bumblebee}
    \riskdescription{L'operatore preesistente non risciuva nativamente a gestire un output informativo che potesse trattare \textbf{gruppi di risorse} (eguali o diverse fra di loro)}
    \risksolution{Si è deciso di modificare la rappresentazione di tale output informativo per garantire la gestione sia di gruppi di risorse che risporse singole, sfruttando quindi un comune tipo di output adattabile alle esigenze appena descritte}
...
\begin{risk}{Adattabilità operatore Anomaly Detector}
    \riskdescription{l'operatore preesistente non riusciva ad alzare anomalie su gruppi di risorse differenti fra di loro e adattarsi all'aggiunta di nuovi detector}
...
\begin{enumerate}
	\item{Studio e analisi preliminare del \gls{framework} Flink;}
	\item{Studio e analisi preliminare del linguaggio di programmazione Scala;}
	\item{Sviluppo di \gls{poc} per compredere al meglio alcune funzionalità essenziali di Flink quali \gls{serializzazione};}
	\item{Sviluppo e modifica degli operatori preesistenti per l'integrazione della componente di \textit{anomaly detection} su asset raggruppati;}
...
\begin{itemize}
	\item{Minimi:
		\begin{itemize}
			\item{Comprensione del workflow e degli strumenti aziendali;}
			\item{Comprensione dei linguaggi e delle architetture coinvolte;}
			\item{Mappatura dell’attuale \gls{pipeline} di analisi degli eventi (Flink);}
			\item{Definizione dell’architettura ad alto livello per la funzionalità location digital twin ed eventuali modifiche al sistema attuale;}
...
\begin{itemize}
	\item{Minimi:
		\begin{itemize}
			\item{Completamento dello sviluppo e validazione dell’operatore di location digital twinning;}
...
Al termine dello stage il prodotto realizzato riesce a gestire integramente l'analisi di gruppi di asset raggruppati, sia per quando riguarda la suddivisione corretta di risorse in gruppi prefissati, sia per quanto concerne l'analisi relativa a se è stata rilevata un'anomalia.\\
Inoltre i test realizzati garantiscono che le modifiche apportate non abbiano intaccato le funzionalità precedenti, cioè quelle relative all'analisi di asset singoli non raggruppati o il raggrummento relativo alla stessa tipologia di risorsa.\\
