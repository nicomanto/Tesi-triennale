...
\hline
TU19 & L'operatore configurato con la modalità \textit{aggregation\_mean} non colleziona il \textit{GroupedEvents} poiché almeno un elemento durante l'aggregazione collettiva di \textit{asset} differenti non conteneva un particolare \textit{DataField} richiesto per l'aggregazione stessa & Superato \\
\hline
...
\hline
TU28 & L'operatore ignora l'aggiornamento della configurazione perché la nuova configurazione è di tipo "\textit{grouping}" ma il \textit{detector} configurato all'interno di essa è di tipo \textit{single input} & Superato \\
\hline
\end{longtable}
}
